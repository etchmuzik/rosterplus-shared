...
| iOS — App Store | 🟡 TestFlight beta | Every primary surface Supabase-backed. Build green, **129 tests** passing (was 116 — added NotificationPrefs decode/DTO/store coverage + Haptics enable-policy). **Build 6 live on TestFlight (VALID in App Store Connect, uploaded 2026-05-30 via Xcode Organizer).** **2026-05-30 batch** (`bd85542`): iOS Settings now persists `notification_prefs` (closes the iOS↔web notification-toggle parity gap — was the last tracked iOS code-side follow-up); Haptics toggle made real (40 call sites gated on `@AppStorage`, was inert). +13 commits past last STATUS: Decimal money fix (`83f2625`), RLS hygiene on artists+bookings deleted_at filters (`423f1e0`, `f23562d`), invitations sessionEpoch guard (`a9f3abf`), claim flow honest CTAs (`1914cc2`), invoice view wired to real data (`81ce2ec`), settings 6 dead rows wired (`b9210b0`), retry CTAs across artist/EPK/booking-detail (`6164962`), optimistic rollback on writer failure across stores (`ea1efcf`, `0aac315`). AASA live; universal links wired. **AR localisation 66 keys.** |
...
- **🟠 Supabase leaked-password protection (HaveIBeenPwned) is disabled — KNOWINGLY DEFERRED at soft-launch (operator decision, 2026-05-30).** Dashboard toggle at Auth → Settings → Password Strength. The platform handles money + contracts, so launching auth without leaked-password checking is an **accepted risk** the operator chose to ship with; flip it as soon as practical post-launch. Was previously the one hard-required launch blocker; downgraded from 🔴 to 🟠 to record that the call was made deliberately, not missed.
...
- ~~**TestFlight build 6.**~~ **DONE 2026-05-30.** Build 6 (commit `bd85542` + version bump `8f9073d`) uploaded via Xcode Organizer, confirmed **VALID** in App Store Connect — live on TestFlight. Ships the notification-prefs persistence + haptics-toggle batch. **Note:** the headless CLI path (`scripts/export.sh`/`upload.sh` with API key `C4TFQQ5AAD`) hit "Cloud signing permission error" — that key lacks Admin/App Manager role to mint an iOS Distribution cert. Organizer GUI succeeded because it signs interactively as the account holder. To unblock headless `ship.sh` in future, grant the ASC key Admin/App Manager access (App Store Connect → Users and Access → Integrations → Keys).
- **App Store submission — SUBMISSION-READY (operator clicks remain).** As of 2026-06-07: Build 6 VALID on TestFlight; **9 screenshots** captured at 1320×2868 (6.9") in `/tmp/rostr-shots/` via the new DEBUG screenshot harness (ETCH-branded demo data); **demo account created + verified** — `appreview@rosterplus.io` (promoter, UUID `e182c045-418f-435f-8e69-e77b785efc43`), seeded with 1 confirmed booking / 1 signed contract / 1 completed payment / 1 thread (login + RLS reads verified against prod). All field copy in `workspace/docs/APP_STORE_METADATA.md`; click-by-click in `workspace/docs/APP_STORE_SUBMISSION_PACKET.md`. **Remaining = App Store Connect web-UI clicks only** (paste metadata, drag screenshots, App Privacy/age-rating questionnaires, paste demo creds, Submit). 6.5" set NOT needed — ASC auto-scales the 6.9" set.
