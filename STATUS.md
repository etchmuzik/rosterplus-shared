...
- **TestFlight build 6.** Build 4 live on TestFlight. Build 6 (commit `bd85542` + version bump) ships the notification-prefs persistence + haptics-toggle batch. Upload via `bash ios/scripts/ship.sh` once ASC API key env vars are set (`ASC_KEY_ID`/`ASC_ISSUER_ID`/`ASC_KEY_PATH`).
